...
fun FenixInstallerApp(
    ...
    Scaffold(
        ...
        LazyColumn(
            ...
            item {
                SearchSection(
                    selectedProject = fenixInstallerViewModel.selectedProject,
            ...
            if (fenixInstallerViewModel.isLoading && fenixInstallerViewModel.selectedJobs.isEmpty() && fenixInstallerViewModel.relevantPushComment == null) {
                item { LoadingState() }
            }

            fenixInstallerViewModel.errorMessage?.let {
                // TODO: Consider creating a specific string resource for \"Download failed\" if it's a common prefix for user-facing errors.
                if (fenixInstallerViewModel.selectedJobs.isEmpty() || !it.startsWith("Download failed")) {
                    item { ErrorState(errorMessage = it) }
                }
            }

            fenixInstallerViewModel.relevantPushComment?.let { comment ->
                if (comment.isNotBlank() || fenixInstallerViewModel.relevantPushAuthor != null) { // Show card if comment or author exists
                    item {
                        PushCommentCard(
                            comment = comment ?: "",
                            author = fenixInstallerViewModel.relevantPushAuthor,
                            revision = fenixInstallerViewModel.revision // Added revision
                        )
                    }
                }
            }

            if (fenixInstallerViewModel.selectedJobs.isNotEmpty()) {
                item {
                    Text(
                        text = stringResource(id = R.string.treeherder_apks_jobs_found_message, fenixInstallerViewModel.selectedJobs.size),
                ...
                items(fenixInstallerViewModel.selectedJobs, key = { it.taskId }) { job ->
                    AppCard(job = job, viewModel = fenixInstallerViewModel)
                }
            } else if (!fenixInstallerViewModel.isLoading && fenixInstallerViewModel.errorMessage == null && (fenixInstallerViewModel.relevantPushComment != null || fenixInstallerViewModel.relevantPushAuthor != null)) {
                 // Slightly adjusted logic to account for author possibly being present even if comment is not
                if (fenixInstallerViewModel.relevantPushComment?.isNotBlank() == true || fenixInstallerViewModel.relevantPushAuthor != null ) {
