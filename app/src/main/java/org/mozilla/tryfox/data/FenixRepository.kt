...
class FenixRepository(
    ...
    override suspend fun getPushByRevision(
        project: String,
        revision: String,
    ): NetworkResult<TreeherderRevisionResponse> = safeApiCall { treeherderApiService.getPushByRevision(project, revision) }

    override suspend fun getPushesByAuthor(author: String): NetworkResult<TreeherderRevisionResponse> =
        safeApiCall {
            treeherderApiService.getPushByAuthor(author = author)
        }

    override suspend fun getJobsForPush(pushId: Int): NetworkResult<TreeherderJobsResponse> =
        safeApiCall {
            treeherderApiService.getJobsForPush(pushId)
        }
                ...
                try {
                    ...
                    while (inputStream.read(buffer).also { read = it } != -1) {
                        ...
                        if (bytesDownloaded == 0L ||
                            ...
                            (bytesDownloaded % (totalBytes / 10).coerceAtLeast(1)) == 0L
                        ) {
                    ...
                    logcat(
                        LogPriority.ERROR,
                        TAG,
                    ) { "Exception during file I/O stream operations: ${e.message}\n${Log.getStackTraceString(e)}" }
                    throw e
                } finally {
                    try {
                        inputStream?.close()
