...
class HomeViewModelTest {
    ...
    private fun createTestParsedNightlyApk(
        ...
        val fileName =
            if (appName == testReferenceBrowserAppName) {
    ...
    fun setUp() =
        runTest {
            ...
            whenever(mockMozillaArchiveRepository.getFenixNightlyBuilds()).thenReturn(
                NetworkResult.Success(emptyList()),
            )
            whenever(mockMozillaArchiveRepository.getFocusNightlyBuilds()).thenReturn(
                NetworkResult.Success(emptyList()),
            )
            whenever(mockMozillaArchiveRepository.getReferenceBrowserNightlyBuilds()).thenReturn(
                NetworkResult.Success(emptyList()),
            ) // Added
    ...
    @Test
    fun `initialLoad success should update HomeScreenState to Loaded with data`() =
        runTest {
            ...
            whenever(mockMozillaArchiveRepository.getReferenceBrowserNightlyBuilds()).thenReturn(
                NetworkResult.Success(listOf(rbParsed)),
            ) // Mocked for RB
            ...
            assertTrue(
                loadedState.referenceBrowserBuildsState is ApksState.Success,
                "Reference Browser builds should be Success",
            )
    ...
    @Test
    fun `initialLoad with fenix cache populated should result in IdleNonEmpty cache state from CacheManager`() =
        runTest {
            val fenixParsed = createTestParsedNightlyApk(testFenixAppName, testDateRaw, testVersion, testAbi)
    ...
    @Test
    fun `clearAppCache should call CacheManager and update states to NotDownloaded`() =
        runTest {
            val fenixParsed = createTestParsedNightlyApk(testFenixAppName, testDateRaw, testVersion, testAbi)
    ...
    @Test
    fun `downloadNightlyApk success sequence`() =
        runTest {
            val fenixParsed = createTestParsedNightlyApk(testFenixAppName, testDateRaw, testVersion, testAbi)
    ...
    @Test
    fun `downloadNightlyApk failure sequence`() =
        runTest {
            val fenixParsed = createTestParsedNightlyApk(testFenixAppName, testDateRaw, testVersion, testAbi)
            val apkToDownload = createTestApkUiModel(fenixParsed, DownloadState.NotDownloaded)
            val expectedApkDir = File(tempCacheDir, "${apkToDownload.appName}/${apkToDownload.date.substring(0, 10)}")
