...
class MozillaArchiveRepositoryImplTest {
    ...
    fun `getFenixNightlyBuilds success - single latest build`() = runTest {
        ...
        whenever(mockMozillaArchivesApiService.getHtmlPage(eq(expectedUrl))).thenReturn(mockHtml)
    ...
    fun `getFenixNightlyBuilds network error`() = runTest {
        ...
        val expectedUrl = "https://archive.mozilla.org/pub/fenix/nightly/2023/10/"
        whenever(mockMozillaArchivesApiService.getHtmlPage(eq(expectedUrl))).thenThrow(RuntimeException(errorMessage))
    ...
    fun `getFenixNightlyBuilds when current month 404 queries previous month`() = runTest {
        ...
        whenever(mockMozillaArchivesApiService.getHtmlPage(currentMonthUrl)).thenThrow(
            HttpException(Response.error<Any>(404, "".toResponseBody(null))),
        )
        whenever(mockMozillaArchivesApiService.getHtmlPage(previousMonthUrl)).thenReturn(previousMonthMockHtml)
