...
class MozillaArchiveRepositoryImplTest {
    ...
    private val fenixFullUrl1 =
        "https://archive.mozilla.org/pub/fenix/nightly/$YEAR/$MONTH/$fenixDirString1$fenixFileName1"
    ...
    private val fenixFullUrl2 =
        "https://archive.mozilla.org/pub/fenix/nightly/$YEAR/$MONTH/$fenixDirString2$fenixFileName2"
    ...
    private val focusFullUrl =
        "https://archive.mozilla.org/pub/focus/nightly/$YEAR/$MONTH/$focusDirString$focusFileName"
    ...
    private fun createMockHtmlResponse(vararg dirStrings: String): String =
        dirStrings.joinToString(separator = "\n") {
            """<td>Dir</td><td><a href="$it">$it</a></td>"""
        }

    @Test
    fun `getFenixNightlyBuilds success - single latest build`() =
        runTest {
    ...
    @Test
    fun `getFenixNightlyBuilds success - multiple builds on latest date`() =
        runTest {
            ...
            val expectedApks =
                listOf(
                    ParsedNightlyApk(
                        originalString = fenixDirString1,
                        rawDateString = DATE,
                        appName = FENIX,
                        version = fenixVersion,
                        abiName = "arm64-v8a",
                        fullUrl = fenixFullUrl1,
                        fileName = fenixFileName1,
                    ),
                    ParsedNightlyApk(
                        originalString = fenixDirString2,
                        rawDateString = DATE,
                        appName = FENIX,
                        version = fenixVersion,
                        abiName = "x86_64",
                        fullUrl = fenixFullUrl2,
                        fileName = fenixFileName2,
                    ),
                )
            assertEquals(expectedApks.toSet(), apks.toSet())
        }

    @Test
    fun `getFenixNightlyBuilds success - sorts by date correctly`() =
        runTest {
            val olderDateDir = "2023-09-30-23-59-59-$FENIX-$fenixVersion-android-arm64-v8a/"
            val middleDateDir = fenixDirString1
            val latestDateDir = "2023-10-01-11-00-00-$FENIX-$fenixVersion-android-x86/"
    ...
    @Test
    fun `getFenixNightlyBuilds success - no builds found`() =
        runTest {
    ...
    @Test
    fun `getFenixNightlyBuilds network error`() =
        runTest {
            ...
            val expectedMessage = "Failed to fetch or parse fenix builds: $errorMessage"
            assertEquals(expectedMessage, (result as NetworkResult.Error).message)
        }

    @Test
    fun `getFenixNightlyBuilds when current month 404 queries previous month`() =
        runTest {
            // Given
            ...
            val previousMonthDirString =
                "$previousMonthDateString-$FENIX-$fenixVersion-android-arm64-v8a/"
    ...
    @Test
    fun `getFocusNightlyBuilds success - single latest build`() =
        runTest {
    ...
    @Test
    fun `getFocusNightlyBuilds network error`() =
        runTest {
            ...
            assertTrue(result is NetworkResult.Error)
            val expectedMessage = "Failed to fetch or parse focus builds: $errorMessage"
            assertEquals(expectedMessage, (result as NetworkResult.Error).message)
        }
}
